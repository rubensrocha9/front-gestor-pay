...
export class ExpenseManagerComponent implements OnInit {
  ...
  getExpenseManager(): void {
    ...
    this.expenseManagerService.getPageParams(this.companyId, this.pagination.currentPage, this.pagination.itemsPerPage).pipe(take(1)).subscribe(
      data => {
  ...
  searchField(evt: any): void {
    if (this.searchFieldChanged.observers.length === 0) {
      ...
      this.searchFieldChanged.pipe(debounceTime(800)).subscribe(
        filtered => {
          LoaderService.toggle({ show: true });
          this.expenseManagerService.getPageParams(this.companyId, this.pagination.currentPage, this.pagination.itemsPerPage, filtered).pipe(take(1)).subscribe(
            data => {
              LoaderService.toggle({ show: false });
              this.managers = data.result;
              this.pagination = data.pagination;
  ...
  onEdit(id?: number): void {
    this.router.navigateByUrl(`expense-manager/detail/${id}`);
  }

  onDelete(id?: number, evt?: any) {
    evt.stopPropagation();
    this.modal.confirm({
      nzTitle: '<i>Certeza que deseja remover esse gasto?</i>',
      nzOnOk: () => {
        this.expenseManagerService.delete(this.companyId, id!).pipe(take(1)).subscribe(
        data => {
          this.getExpenseManager();
          this.message.success('Gasto removido com sucesso!', { nzDuration: 3000 });
        }, error =>{
          this.notificationService.modalLoadDataError(error);
        })
      }
